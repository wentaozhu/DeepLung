...
def evaluateCAD(seriesUIDs, results_filename, outputDir, allNodules, CADSystemName, maxNumberOfCADMarks=-1,
                performBootstrapping=False,numberOfBootstrapSamples=1000,confidence = 0.95, vis=True, delimiter=',', clr = 'b', cap=''):
    '''
    ...
    for seriesuid in seriesUIDs:
        ...
        for result in results[1:]:
            # print(seriesuid_label, header.index(seriesuid_label), result)#, header.index(0))
            nodule_seriesuid = result[header.index(seriesuid_label)] #[0].split(',')
            # print(nodule_seriesuid, nodule_seriesuid[0])
            if seriesuid[0] == '0':
                nodule_seriesuid = '{:05d}'.format(int(nodule_seriesuid))
            # print(nodule_seriesuid, seriesuid_label, seriesuid)
            ...
            if len(nodules.keys()) > maxNumberOfCADMarks:
                ...
                for keytemp, noduletemp in nodules.iteritems():
                    ...
                    if float(noduletemp.CADprobability) > probThreshold:
                        ...
                        nrNodules2 += 1
                nodules = nodules2
        # print 'adding candidates: ' + seriesuid, len(nodules)
    ...
    # -- loop over the cases
    for seriesuid in seriesUIDs:
        # print(seriesuid, len(allCandsCAD[seriesuid].keys()))
        ...
        try:
            ...
            candidates = {}
        # print(len(candidates), seriesuid, len(allCandsCAD))
    ...
    # compute FROC
    # print(sum(FROCProbList), len(FROCProbList))#, excludeList)
    ...
    if performBootstrapping:# and vis:
    ...
    for fp, se in zip(fps_itp, sens_itp):# fps_bs_itp sens_bs_mean):
        if fp >= curfp:
            ...
            if curfp == 16:
                break
    return frocv/7 #(fps, sens, thresholds, fps_bs_itp, sens_bs_mean, sens_bs_lb, sens_bs_up)
    
def getNodule(annotation, header, state = ""):
    nodule = NoduleFinding()
    # print(header, coordX_label, annotation)
...
def noduleCADEvaluation(annotations_filename,annotations_excluded_filename,seriesuids_filename,results_filename,outputDir, vis=False, delimiter=',', clr = 'b', cap=''):
    '''
    ...
    return evaluateCAD(seriesUIDs, results_filename, outputDir, allNodules,
                ...
                numberOfBootstrapSamples=bNumberOfBootstrapSamples, confidence=bConfidence, vis=vis, delimiter=delimiter, clr = clr, cap=cap)
...
if __name__ == '__main__':
    ...
    annotations_filename = '../evaluationScript/annotations/newannotations.csv' #annotations.csv'
